var w=800;                                            // Ancho del fondo
var h=400;                                           // Alto del fondo
var jugador;                                        // Variable para llamar al Jugador
var fondo;    

var bala,  disp=false,   nave;                    // Objeto Nave_Horizontal                   
var bala2, disp2=false, nave2;                  // Objeto Nave_Vertical
var bala3, disp3=false, nave3;                 // Objeto Nave_Diagonal
...
// Estatus arriba, abajo, derecha e izquierda
var arriba;
var abajo;
var derecha;
var izquierda;
...
function create() {
    ...
    nnNetwork =  new synaptic.Architect.Perceptron(6,6,8,4);    // -> Parametros (6,6,6,4), (6,3,3,4), (6,5,5,4)  -> Se comporta 2/3 (6,5,5,5,4)
    nnEntrenamiento = new synaptic.Trainer(nnNetwork); 
} 
                                               //    0.0003               35000    ->   parametro 0.0003 y 4000 con (6,5,5,4)
function enRedNeural(){                       //     0.0001               25000  40000  25000
...
function update() {
    ...
    if(!jugador.body.onFloor() || jugador.body.velocity.y !=0) {   // Detecta si el jugador brincando
        abajo = 0;
        arriba = 1;
    }
    
    if(jugador.body.velocity.x >= 140) {                         // El Jugador esta corriendo
        derecha = 1;
        izquierda = 0;
    }

    // Calcula la diferencia horizontal (en el eje ) entre la posición del jugador y la posición de la primera bala.
    ...
    if(jugador.position.x >= 200) atras();   // El jugador no pude avanzar mas de 200px 
    
    if( modoAuto==false && salto.isDown &&  !arriba ){  // Avanza derecha, salta solo si no esta en el borde 200px
    ...
    if( modoAuto==false && avanzar.isDown &&  jugador.body.onFloor()) correr();  // Avanza derecha
    if( modoAuto==false && jugador.body.onFloor() && jugador.position.x >= 200)  atras(); // Avanza Izquierda
    if( modoAuto==false && !avanzar.isDown &&  jugador.body.onFloor() && jugador.position.x == 50 )  Retrceder(); // Mantener Pos_inicial
...
function disparo(){
    velocidadBala =  -1 * velocidadRandom(300,700); //     300 700
...
function disparo2(){
    velocidadBala2 =  -1 * velocidadRandom(300,800 ); //   200 800
    bala2.body.velocity.y = 0 ;
    disp2=true;
}

function disparo3(){
    velocidadBala3 =  -1 * velocidadRandom(400,500); // 400,500    100 500
...
function saltar(){
    if(jugador.position.x >= 200){
        atras();  // Si esta en eje x=200 y brinca, regresa a la pos_ini
    }else{
        jugador.body.velocity.y = -270;   // si no, le perite hacer el salto
    }
}
